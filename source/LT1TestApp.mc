...
class LT1TestApp extends Application.AppBase {
    ...
    function onStart(state as Dictionary?) as Void {
        ...
        sensor      = new SensorLayer(rrBuffer);
        ...
        orchestrator = new TestOrchestrator(
            sensor, rrBuffer, dfaComputer, lt1Detector, fitRecorder, stageGuide
        );
        ...
        // Start sensor NOW (before test start) so chest-strap vs optical
        // detection resolves during the idle screen.  start() is idempotent,
        // so calling it again in startTest() is harmless.
        sensor.start();
    }
